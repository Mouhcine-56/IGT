...
def Solve_MFG(obs, V_NN, G_NN, num_epoch, t, lr, num_samples, device):
    ...
    Inspiré de Solve_MFG2 dans le fichier de référence.
    """
    V_NN.train()
    G_NN.train()
...
#         t_start = 0.0# if np.random.rand() < 0.5 else np.random.uniform(0, obs.TT * 0.9)
...
def wasserstein_fp(obs, G_NN_list,              # BR passés (longueur k)
    ...
    for j in range(T):
        # population : x0[:, :2] + BR passés (chacun N pts)
        if obs.Round == 0:
            parts = [m0[:, :2]]                              # (N,2)
        else:
            parts = [] 
